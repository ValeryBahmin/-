...
Логика запроса:
   ...
   В основном запросе присоединяем к таблице flights таблицу airports по кодам аэропорта вылета и аэропорта прилета. 
   Присоединяем таблицу aircrafts по коду самолета. 
...
Логика запроса:
   ...
   Фильтруем полученную таблицу, выбирая только строки со значением NULL для номера посадочного талона.
...
/* Вопрос №9: Вычислите расстояние между аэропортами, связанными прямыми рейсами, сравните с допустимой 
